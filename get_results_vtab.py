...
root = "/home/ch7858/vpt/output_finalfinal/vtab-dsprites(predicted_attribute=label_x_position,num_classes=16)_P5_VK5_SHARED_1_INIT_0_ACC_0"
...
#     model_type = f"adapter_{r}"
    ...
    # print('!!!', files)
    for f in files:
        df = get_df(files, f"run{run}", root, MODEL_NAME, is_best=False, is_last=True, dataset_type=dataset_type)
        if df is None:
            continue
        df["seed"] = seed
    df_list.append(df)
