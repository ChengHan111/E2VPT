...
SavedPath = FolderName.split('output_folderACC/')[1]
# plt.figure(dpi=300)
...
for folder in subfolders:
    ...
    for file_path in txt_files:
        ...
        val_loss = read_loss_from_txt(file_path, 0)
        if task.endswith('TwoSteps'):
            if counter_train % 5 == 0:
                counter_train += 1
        ...
        plt.legend()
        plt.title(os.path.basename('Val_ACC'))
        plt.savefig(f'output_folderACC/{SavedPath}/Val_ACC.png')
...
for folder in subfolders:
    ...
    for file_path in txt_files:
        ...
        test_loss = read_loss_from_txt(file_path, 1)
        if task.endswith('TwoSteps'):
            if counter_train % 5 == 0:
                counter_train += 1
        ...
        plt.legend()
        plt.title(os.path.basename('Test_ACC'))
        plt.savefig(f'output_folderACC/{SavedPath}/Test_ACC.png')
